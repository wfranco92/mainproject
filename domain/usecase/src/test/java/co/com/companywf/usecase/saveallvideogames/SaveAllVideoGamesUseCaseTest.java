...
class SaveAllVideoGamesUseCaseTest {
    ...
    @InjectMocks
    private SaveAllVideoGamesUseCase saveAllVideoGamesUseCase;
    ...
    void saveVideoGames(){
        ...
        when(videogameRepository.saveAllVideoGames(any())).thenReturn(Flux.just(Videogame.builder().id("AAA").name("videogame").gender("Rol").status("Jugado").developer("Ubisoft").location("Otro").createdAt(LocalDateTime.now()).build()));

        StepVerifier.create(saveAllVideoGamesUseCase.execute(List.of(videoGameRequest)))
